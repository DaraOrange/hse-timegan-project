...
class Discriminator(nn.Module):
    def __init__(self, input_dim, hidden_dim, device, n_layers=1):
        super().__init__()
        self.d_cell = nn.GRU(input_size=input_dim, hidden_size=hidden_dim, num_layers=n_layers, batch_first=True)
        self.tanh = nn.Tanh()
    ...
    def __call__(self, x, T):
        ...
        x_packed = x_packed.float()#to(self.device)
...
def discriminative_score_metrics (ori_data, generated_data, device):
    ...
    discriminator = Discriminator(input_dim=dim, hidden_dim=hidden_dim, device=device).to(device)
    ...
    y_pred_real_curr = y_pred_real_curr.detach().numpy()
    y_pred_fake_curr = y_pred_fake_curr.detach().numpy()
